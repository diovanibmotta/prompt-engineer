...
from src.llm_provider import get_eval_llm
...
def evaluate_metric(metric_name: str, 
                   prompt_output: str, 
                   reference_output: str,
                   bug_report: str) -> tuple:
    """
    Função genérica para avaliar qualquer métrica usando LLM-as-Judge.
    
    Orquestra a avaliação de diferentes métricas baseada no nome da métrica.
    Cada métrica tem critérios de avaliação específicos e retorna um score
    entre 0.0 e 1.0 com reasoning explicando a avaliação.
    
    Métricas suportadas:
    - "clarity": Clareza e estrutura da resposta
    - "precision": Precisão e ausência de alucinações
    - "f1_score": Balanceamento entre precision e recall
    - "helpfulness": Utilidade da resposta (Clarity + Precision) / 2
    - "correctness": Correção da resposta (F1-Score + Precision) / 2
    - "tone_score": Tom profissional e empático (específico para Bug to User Story)
    - "acceptance_criteria_score": Qualidade dos critérios de aceitação
    - "user_story_format_score": Conformidade com formato padrão de user story
    - "completeness_score": Completude em relação ao bug original
    
    Args:
        metric_name: Nome da métrica a avaliar (case-insensitive)
        prompt_output: Saída gerada pelo prompt otimizado
        reference_output: Saída esperada/referência (ground truth)
        bug_report: Relatório do bug original (contexto)
    
    Returns:
        tuple: (score, reasoning) onde:
            - score (float): Valor entre 0.0 e 1.0
            - reasoning (str): Explicação da avaliação
    
    Raises:
        ValueError: Se métrica não for suportada
        
    Examples:
        >>> score, reasoning = evaluate_metric(
        ...     "clarity",
        ...     "User story gerada",
        ...     "User story esperada",
        ...     "Bug report"
    ...
    **Validates: Requirements 6.1, 6.2, 6.3, 6.4, 6.5, 6.6, 6.7, 6.8, 6.9, 6.10**
    ...
    # Mapeamento de métricas para funções de avaliação
    metric_functions = {
        "clarity": evaluate_clarity,
        "precision": evaluate_precision,
        "f1_score": evaluate_f1_score,
        "f1-score": evaluate_f1_score,
        "tone_score": evaluate_tone_score,
        "tone-score": evaluate_tone_score,
        "acceptance_criteria_score": evaluate_acceptance_criteria_score,
        "acceptance-criteria-score": evaluate_acceptance_criteria_score,
        "user_story_format_score": evaluate_user_story_format_score,
        "user-story-format-score": evaluate_user_story_format_score,
        "completeness_score": evaluate_completeness_score,
        "completeness-score": evaluate_completeness_score,
    }
    
    # Verificar se métrica é suportada
    if metric_name_lower not in metric_functions:
        supported_metrics = ", ".join(sorted(set(metric_functions.keys())))
        raise ValueError(
            f"❌ Métrica '{metric_name}' não suportada.\n"
            f"Métricas disponíveis: {supported_metrics}"
        )
    
    # Obter função de avaliação
    eval_function = metric_functions[metric_name_lower]
    
    try:
        # Chamar função de avaliação específica
        result = eval_function(bug_report, prompt_output, reference_output)
        
        # Extrair score e reasoning
        score = float(result.get("score", 0.0))
        reasoning = str(result.get("reasoning", ""))
        
        # Validar score está entre 0.0 e 1.0
        if not (0.0 <= score <= 1.0):
            print(f"⚠️  Score fora do intervalo [0.0, 1.0]: {score}. Ajustando para 0.0")
            score = 0.0
        
        return (score, reasoning)
        
    except Exception as e:
        print(f"❌ Erro ao avaliar métrica '{metric_name}': {str(e)}")
        return (0.0, f"Erro na avaliação: {str(e)}")
...
def calculate_f1_score(precision: float, recall: float) -> float:
    """
    Calcula o F1-Score a partir de Precision e Recall.

    F1-Score = 2 * (Precision * Recall) / (Precision + Recall)

    Args:
        precision: Valor de precisão entre 0.0 e 1.0
        recall: Valor de recall entre 0.0 e 1.0

    Returns:
        float: F1-Score entre 0.0 e 1.0

    Raises:
        ValueError: Se precision ou recall não estão entre 0.0 e 1.0
    """
    if not (0.0 <= precision <= 1.0):
        raise ValueError(f"Precision deve estar entre 0.0 e 1.0, recebido: {precision}")
    if not (0.0 <= recall <= 1.0):
        raise ValueError(f"Recall deve estar entre 0.0 e 1.0, recebido: {recall}")
...
def calculate_helpfulness(clarity: float, precision: float) -> float:
    """
    Calcula a métrica de Helpfulness (Utilidade).

    Helpfulness = (Clarity + Precision) / 2

    Mede se a resposta é útil ao usuário, combinando:
    - Clarity: Quão clara e bem estruturada é a resposta
    - Precision: Quão precisa e correta é a resposta

    Args:
        clarity: Valor de clareza entre 0.0 e 1.0
        precision: Valor de precisão entre 0.0 e 1.0

    Returns:
        float: Helpfulness entre 0.0 e 1.0

    Raises:
        ValueError: Se clarity ou precision não estão entre 0.0 e 1.0
    """
    if not (0.0 <= clarity <= 1.0):
        raise ValueError(f"Clarity deve estar entre 0.0 e 1.0, recebido: {clarity}")
    if not (0.0 <= precision <= 1.0):
        raise ValueError(f"Precision deve estar entre 0.0 e 1.0, recebido: {precision}")

    helpfulness = (clarity + precision) / 2
    return round(helpfulness, 4)


def calculate_correctness(f1_score: float, precision: float) -> float:
    """
    Calcula a métrica de Correctness (Correção).

    Correctness = (F1-Score + Precision) / 2

    Mede se a resposta está correta, combinando:
    - F1-Score: Balanceamento entre precision e recall
    - Precision: Ausência de informações incorretas

    Args:
        f1_score: Valor de F1-Score entre 0.0 e 1.0
        precision: Valor de precisão entre 0.0 e 1.0

    Returns:
        float: Correctness entre 0.0 e 1.0

    Raises:
        ValueError: Se f1_score ou precision não estão entre 0.0 e 1.0
    """
    if not (0.0 <= f1_score <= 1.0):
        raise ValueError(f"F1-Score deve estar entre 0.0 e 1.0, recebido: {f1_score}")
    if not (0.0 <= precision <= 1.0):
        raise ValueError(f"Precision deve estar entre 0.0 e 1.0, recebido: {precision}")
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("PARTE 0: MÉTRICAS DE CÁLCULO BÁSICO")
    print("=" * 70)

    # Teste das métricas de cálculo básico
    print("\n1. Calculate F1-Score:")
    f1_calc = calculate_f1_score(0.9, 0.85)
    print(f"   F1-Score(precision=0.9, recall=0.85) = {f1_calc}")
    assert 0.0 <= f1_calc <= 1.0, "F1-Score deve estar entre 0.0 e 1.0"
    print(f"   ✓ Validado: {f1_calc} está entre 0.0 e 1.0\n")

    print("2. Calculate Helpfulness:")
    helpfulness = calculate_helpfulness(0.92, 0.88)
    print(f"   Helpfulness(clarity=0.92, precision=0.88) = {helpfulness}")
    assert 0.0 <= helpfulness <= 1.0, "Helpfulness deve estar entre 0.0 e 1.0"
    print(f"   ✓ Validado: {helpfulness} está entre 0.0 e 1.0\n")

    print("3. Calculate Correctness:")
    correctness = calculate_correctness(0.87, 0.91)
    print(f"   Correctness(f1_score=0.87, precision=0.91) = {correctness}")
    assert 0.0 <= correctness <= 1.0, "Correctness deve estar entre 0.0 e 1.0"
    print(f"   ✓ Validado: {correctness} está entre 0.0 e 1.0\n")
